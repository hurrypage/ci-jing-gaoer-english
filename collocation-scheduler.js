(() => {
  ...
  function dueTasks(limit=4){const s=state();return Object.entries(s.records).filter(([id,r])=>!s.retiredIds[id]&&r.dueAt<=day()).sort((a,b)=>a[1].dueAt.localeCompare(b[1].dueAt)).slice(0,limit).map(([id,r])=>({id,phrase:r.phrase,dueAt:r.dueAt,lastGrade:r.lastGrade}));}
  ...
  async function init(){try{const r=await fetch('data/collocation-catalog-v1.json');if(!r.ok)throw Error();catalog=await r.json();window.CiJingCollocationScheduler={startCourseCards,record,snapshot,getDueTasks:dueTasks,getCatalog:()=>catalog};emit();}catch{console.warn('Collocation catalog could not be loaded.');}}
